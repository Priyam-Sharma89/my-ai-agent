...
#############Why this main4- To Add Password hashing 
...
fake_users_db = {
    'rahul': hash_passwords("password123"),
    'priyam': hash_passwords("mypassword")
}
...
def login(request:LoginRequest):
    if request.username not in fake_users_db:
        raise HTTPException(
            status_code=401,
            detail="Invalid username or password" # We Add the same line because if any one is wrong so hacker know nothing 
        )
    
    # Verify password which hashed 
    ...
    # We removed the wrong condition see upper 
